...
QUnit.test( 'FlagSet functions', function( assert ) {
	...
	assert.equal( flags.setAll( 8 ), 255, 'set the first eight bits......' );
	...
	assert.equal( flags.unflag( 0 ), 254, 'unset bit 0......' );
	assert.equal( flags.count(), 7, 'should be seven bits set......' );
	assert.equal( flags.unflag( 2 ), 250, 'unset bit 2......' );
	assert.equal( flags.count(), 6, 'should be six bits set......' );
	assert.equal( flags.clear(), 0, 'unset all bits......' );
	assert.equal( flags.count(), 0, 'should be zero bits set......' );
	assert.equal( flags.flag( 2 ), 4, 'set bit 2......' );
